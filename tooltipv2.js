...
function tipTextLine2(data) {
  ...
  <strong style="font-size:12pt;">for ${months[month-1]} ${year}</strong>
  <br/><br/>
...
function tipTextMap(data, month, year) {
  ...
  // var styles = Array.from(document.styleSheets).find((d) => {
  //   return d.ownerNode.id === 'custom-style'
  // })

  Array.from(styles.cssRules).find(d => d.selectorText == '.timeframe::before').style['box-shadow'] = boxShadow(data)
  Array.from(styles.cssRules).find(d => d.selectorText == '.timeframe::after').style['box-shadow'] = boxShadow(data)
  // debugger
